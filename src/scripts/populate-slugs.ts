...
const generateSlug = (name: string): string => {
	...
	return name
		.toLowerCase()
		.replaceAll(/[^\d\sa-z-]/g, '') // Remove special characters except spaces and hyphens
		.replaceAll(/\s+/g, '-') // Replace spaces with hyphens
		.replaceAll(/-+/g, '-') // Collapse multiple consecutive hyphens to single
		...
		for (const item of itemsWithoutSlugs) {
			...
			while (!slugFound && counter <= 1000) {
				try {
					...
					slug = `${baseSlug}-${item.id.slice(0, 8)}`;
					slugFound = true;
				}
			}
