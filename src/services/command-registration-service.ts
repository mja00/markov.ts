...
export class CommandRegistrationService {
	...
	public async process(
		...
		const remoteCmds = (await this.rest.get(getCommandsRoute)) as RESTGetAPIApplicationCommandsResult;

		const localCmdsOnRemote = localCmds.filter(localCmd => remoteCmds.some(remoteCmd => remoteCmd.name === localCmd.name));
