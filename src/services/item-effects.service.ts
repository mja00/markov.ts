...
import { Item, inventory, items } from '../db/schema.js';
...
export class ItemEffectsService {
	...
	public async getPassiveItems(userId: string): Promise<Item[]> {
		...
		try {
			...
			throw new Error(`Failed to get passive items: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async getBestConsumableRarityBoost(userId: string): Promise<{ item: Item; inventoryCount: number; } | null> {
		...
		try {
			...
			throw new Error(`Failed to get best consumable: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
