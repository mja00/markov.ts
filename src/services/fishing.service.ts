...
export class FishingService {
	...
	public async pickCatchableByRarity(rarity: Rarity, timeOfDay?: TimeOfDay): Promise<Catchable | null> {
		...
		try {
			...
			throw new Error(`Failed to pick catchable: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async isFirstCatch(catchableId: string): Promise<boolean> {
		...
		try {
			...
			throw new Error(`Failed to check first catch: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async markFirstCatch(catchableId: string, userId: string): Promise<Catchable> {
		...
		try {
			...
			throw new Error(`Failed to mark first catch: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async addCatch(userId: string, catchableId: string): Promise<CatchInsert> {
		...
		try {
			...
			throw new Error(`Failed to add catch: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async getAllCatchables(): Promise<Catchable[]> {
		...
		try {
			...
			throw new Error(`Failed to get catchables: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async getCatchableById(catchableId: string): Promise<Catchable | null> {
		...
		try {
			const result = await db.select().from(catchables).where(eq(catchables.id, catchableId))
				.limit(1);

			return result[0] || null;
		} catch (error) {
			Logger.error(`[FishingService] Failed to get catchable ${catchableId}:`, error);
			throw new Error(`Failed to get catchable: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async getUserCatchHistory(
		...
		try {
			...
			throw new Error(`Failed to get catch history: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async getCatchableStats(catchableId: string): Promise<{
		...
		try {
			...
			throw new Error(`Failed to get catchable stats: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
