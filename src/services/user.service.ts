...
export class UserService {
	...
	public async ensureUserExists(discordSnowflake: string, discordTag?: string): Promise<User> {
		...
		try {
			...
			throw new Error(`Failed to ensure user exists: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async getUserBySnowflake(discordSnowflake: string): Promise<User | null> {
		...
		try {
			...
			throw new Error(`Failed to get user: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async addMoney(userId: string, amount: number): Promise<User> {
		...
		try {
			...
			throw new Error(`Failed to add money: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async setMoney(userId: string, amount: number): Promise<User> {
		...
		try {
			...
			throw new Error(`Failed to set money: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async getUserStats(userId: string): Promise<{
		...
		try {
			...
			throw new Error(`Failed to get user stats: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async getTopUsersByMoney(limit: number = 10): Promise<User[]> {
		...
		try {
			return await db.select().from(users).orderBy(desc(users.money))
				.limit(limit);
		} catch (error) {
			Logger.error('[UserService] Failed to get top users by money:', error);
			throw new Error(`Failed to get top users: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
	...
	public async toggleAutoFishing(userId: string): Promise<User> {
		...
		try {
			...
			throw new Error(`Failed to toggle auto fishing: ${error instanceof Error ? error.message : 'Unknown error'}`, { cause: error });
		}
	}
}
