...
describe('FormatUtils', () => {
	...
	describe('duration', () => {
		...
		it('should format hours correctly', () => {
			const result = FormatUtils.duration(3_600_000, Locale.EnglishUS);
			expect(result).toContain('1');
			expect(result.toLowerCase()).toContain('hour');
		});

		it('should format days correctly', () => {
			const result = FormatUtils.duration(86_400_000, Locale.EnglishUS);
			expect(result).toContain('1');
		...
		it('should format mixed duration correctly', () => {
			...
			const result = FormatUtils.duration(5_445_000, Locale.EnglishUS);
		...
		it('should handle very large durations', () => {
			// 365 days
			const result = FormatUtils.duration(31_536_000_000, Locale.EnglishUS);
			expect(result).toBeTruthy();
			expect(result.length).toBeGreaterThan(0);
		});
	...
	describe('fileSize', () => {
		...
		it('should format megabytes correctly', () => {
			const result = FormatUtils.fileSize(1_048_576);
			expect(result).toContain('MB');
		});

		it('should format gigabytes correctly', () => {
			const result = FormatUtils.fileSize(1_073_741_824);
		...
		it('should handle large file sizes', () => {
			const result = FormatUtils.fileSize(1_099_511_627_776); // 1 TB
