...
describe('FishingService Integration Tests', () => {
	...
	describe('pickCatchableByRarity', () => {
		...
		it('should pick different catchables from a pool', async () => {
			...
			const picks = await Promise.all(
				Array.from({ length: 20 }, () => fishingService.pickCatchableByRarity(Rarity.UNCOMMON)),
			);
	...
	describe('determineRarity (without user effects)', () => {
		...
		it('should produce common rarity most frequently', async () => {
			...
			const commonCount = results.filter(rarity => rarity === Rarity.COMMON).length;
			const uncommonCount = results.filter(rarity => rarity === Rarity.UNCOMMON).length;
			const rareCount = results.filter(rarity => rarity === Rarity.RARE).length;
			const legendaryCount = results.filter(rarity => rarity === Rarity.LEGENDARY).length;
